...
async def create_user_to_user_transaction(db:AsyncSession, sender_id: UUID, transaction_data: TransactionCreate):
    ...
    stmt_sender = select(Balance).where(Balance.user_id == sender_id,
    ...
    stmt_receiver = select(Balance).where(Balance.user_id == receiver.id,
                                          Balance.currency_id == transaction_data.currency_id)
    ...
    if not receiver_balance:
        receiver_balance = Balance(user_id =receiver.id,
